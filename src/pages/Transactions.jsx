...
const Transactions = () => {
    ...
    // 🚀 LÓGICA DE ASIENTOS COMPUESTOS (LIBRO DIARIO REAL PARA AUDITORES) 🚀
    const resolveAccountingJournal = (t) => {
        ...
        // 1. Caso crítico: Cruces de Cuentas (isAccounting = true) -> Amortizaciones y retenciones.
        // En la vista fusionada (_isMerged), esto extrae de dónde sale el dinero (Crédito) y a dónde va (Débito).
        if (t._isMerged && t.isInternalTransfer) {
            const debitCode = t._destAccount?.code || 'N/A';
            const debitName = t._destAccount?.name || 'N/A';
            const creditCode = t._sourceAccount?.code || 'N/A';
            const creditName = t._sourceAccount?.name || 'N/A';
            
            // Asiento contable perfecto: un débito y un crédito por el mismo monto
            journalEntries.push({ date, vId, code: debitCode, name: debitName, description: t.description, debit: t._rawAmount, credit: 0 });
            journalEntries.push({ date, vId, code: creditCode, name: creditName, description: t.description, debit: 0, credit: t._rawAmount });
            return journalEntries;
        }

        // 2. Caso: Transacciones de entrada manual avanzada
        ...
        // 3. Caso: Transferencias de dinero simples (entre bancos/caja)
        ...
        // 4. Caso: Ingresos y Egresos Normales (Sin cruces)
        if (!t._isMerged && !t.isInternalTransfer) {
            ...
            if (t.type === 'income') {
                journalEntries.push({ date, vId, code: assetAcc.code, name: assetAcc.name, description: t.description, debit: amount, credit: 0 });
                journalEntries.push({ date, vId, code: catAcc.code, name: catAcc.name, description: t.description, debit: 0, credit: amount });
            } else {
                journalEntries.push({ date, vId, code: catAcc.code, name: catAcc.name, description: t.description, debit: amount, credit: 0 });
                journalEntries.push({ date, vId, code: assetAcc.code, name: assetAcc.name, description: t.description, debit: 0, credit: amount });
            }
        }
        return journalEntries;
    };
        ...
        filteredTransactions.forEach(t => {
            ...
            if (t.isInternalTransfer) {
                ...
                if (sibling) {
                    ...
                    // Asignación correcta de orígenes y destinos basados en cuentas contables
                    const sourceAccountName = expensePart.category === 'Transferencia Interna' ? expensePart.destination.split('|')[1] || expensePart.destination : expensePart.category;
                    const destAccountName = incomePart.category === 'Transferencia Interna' ? incomePart.destination.split('|')[1] || incomePart.destination : incomePart.category;
                    ...
                    // Reemplazamos si es un cruce contable
                    if (expensePart.category !== 'Transferencia Interna') {
                         sourceAsset.name = sourceAccountName;
                         const accSource = (accounts || []).find(a => a.name === sourceAccountName);
                         if (accSource) sourceAsset.code = accSource.number;
                    }
                    if (incomePart.category !== 'Transferencia Interna') {
                         destAsset.name = destAccountName;
                         const accDest = (accounts || []).find(a => a.name === destAccountName);
                         if (accDest) destAsset.code = accDest.number;
                    }
    ...
    const handleSaveTransfer = (transferData) => {
        ...
        if (transferData.isAccounting) {
            // El usuario confirmó que la amortización y las retenciones ocurren AQUÍ.
            // Para el cruce, debitamos de una cuenta (Ej. Anticipo o Ingreso) y acreditamos a otra.
    ...
    // 🚀 LÓGICA DE EXPORTACIÓN PLANA (FLAT DATA) PARA AUDITORÍA
    const handleExport = () => {
        if (displayTransactions.length === 0) return;
        const dataToExport = displayTransactions.map(t => {
            let typeLabel = (t.isInternalTransfer || t.type === 'transfer') ? 'Transferencia/Cruce' : (t.type === 'income' ? 'Ingreso' : 'Egreso');
    ...
    // 🚀 EXPORTACIÓN DEL LIBRO DIARIO DE PARTIDA DOBLE REAL
    const handleExportAccounting = () => {
        ...
        displayTransactions.forEach(t => {
            // Cuando exportamos el diario, no exportamos las filas "ficticias" mergeadas de UI
            // Evaluamos la transacción completa
        ...
        // Verificamos sumas iguales antes de exportar
    ...
    const handlePrint = (t) => {
        ...
        if (t._isMerged) {
            ...
            const entries = resolveAccountingJournal(t);
            const drEntry = entries.find(e => e.debit > 0);
            const crEntry = entries.find(e => e.credit > 0);
            debit = drEntry ? { code: drEntry.code, name: drEntry.name, value: drEntry.debit } : {code:'', name: '', value: 0};
            credit = crEntry ? { code: crEntry.code, name: crEntry.name, value: crEntry.credit } : {code:'', name: '', value: 0};
            
            t._journalEntries = entries;
        }
            ...
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        {canAdd && <Button variant="outline" onClick={() => setTransferDialogOpen(true)}><ArrowRightLeft className="w-4 h-4 mr-2" />Transferir / Cruzar</Button>}
                ...
                <div className="bg-white rounded-xl shadow-sm p-4 border border-slate-200 space-y-4">
                    <div className="flex flex-wrap gap-4 items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            ...
                            <div className="flex bg-slate-100 rounded-lg p-1">
                                <button onClick={() => setViewMode('balances')} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${viewMode === 'balances' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}><TableIcon className="w-3 h-3 inline mr-1" /> Saldos</button>
                                <button onClick={() => setViewMode('accounting')} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${viewMode === 'accounting' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}><BookOpen className="w-3 h-3 inline mr-1" /> Diario</button>
                                <button onClick={() => setViewMode('billing')} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${viewMode === 'billing' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500 hover:text-blue-600'}`}><FileText className="w-3 h-3 inline mr-1" /> Cuentas</button>
                            </div>
                    ...
                    {viewMode !== 'billing' && (
                        <div className="flex gap-2 overflow-x-auto pb-2">
                            {['all', 'income', 'expense', 'transfer'].map(type => (<Button key={type} variant={filterType === type ? 'default' : 'outline'} size="sm" onClick={() => setFilterType(type)} className="capitalize">{type === 'all' ? 'Todas' : type === 'income' ? 'Ingresos' : type === 'expense' ? 'Gastos' : 'Cruces'}</Button>))}
                            <div className="ml-auto flex gap-2">
                                {viewMode === 'accounting' ? <Button variant="outline" size="sm" onClick={handleExportAccounting} className="bg-white shadow-sm"><Download className="w-4 h-4 mr-2" /> Excel (Libro Diario)</Button> : <Button variant="ghost" size="sm" onClick={handleExport}><Download className="w-4 h-4 mr-2" /> Excel</Button>}
                            </div>
                        </div>
                    )}
                </div>
                                                ...
                                                <td className="px-4 py-3 text-center">
                                                    <div className="flex justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        ...
                                                        {t.type === 'expense' && !t.isInternalTransfer && (
                                                            <Button variant="ghost" size="icon" className="h-8 w-8 text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50" onClick={() => handleGenerateBillingDoc(t)} title="Generar Cuenta de Cobro">
                                                                <FileText className="w-3 h-3" />
                                                            </Button>
                                                        )}

                                                        {t.type === 'income' && !t.isInternalTransfer && (
                                                            <Button variant="ghost" size="icon" className="h-8 w-8 text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50" onClick={() => handleGenerateReceipt(t)} title="Generar Recibo de Caja">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="bg-white">
                                    {displayTransactions.map(t => {
                                        // Ignoramos la transacción virtual combinada, resolvemos los asientos reales
                                        if (t._isMerged) return null;
                                        
                                        const journalEntries = resolveAccountingJournal(t);
                                        let rowColorClass = t.type === 'income' ? 'bg-green-50' : (t.type === 'transfer' || t.isInternalTransfer ? 'bg-orange-50' : 'bg-red-50');

                                        return (
                                            <React.Fragment key={t.id}>
                                                {journalEntries.map((entry, idx) => (
                                                    <tr key={idx} className={idx === 0 ? `border-t border-slate-100 ${rowColorClass}` : rowColorClass}>
                                                        <td className="px-4 py-2 text-slate-500">{idx === 0 ? entry.date : ''}</td>
                                                        <td className="px-4 py-2 font-mono text-xs text-slate-400">{idx === 0 ? entry.vId : ''}</td>
                                                        <td className={`px-4 py-2 ${entry.credit > 0 ? 'pl-8' : ''}`}>
                                                            <div className="flex flex-col">
                                                                <span className="font-bold text-slate-700 text-xs">{entry.code}</span>
                                                                <span className="text-slate-600 text-xs uppercase">{entry.name}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-2 text-slate-500 italic text-xs">{entry.description}</td>
                                                        <td className="px-4 py-2 text-right font-mono text-slate-800">{entry.debit > 0 ? entry.debit.toLocaleString('es-CO', { minimumFractionDigits: 2 }) : '-'}</td>
                                                        <td className="px-4 py-2 text-right font-mono text-slate-800">{entry.credit > 0 ? entry.credit.toLocaleString('es-CO', { minimumFractionDigits: 2 }) : '-'}</td>
                                                    </tr>
                                                ))}
                ...
                <DialogContent className="max-w-4xl p-0 border-none bg-transparent shadow-none">
                    <div className="bg-white rounded-lg overflow-hidden shadow-2xl">
                        <div className="p-4 border-b flex justify-between items-center bg-slate-50">
                            ...
                            <Button size="sm" onClick={handlePrintReceiptPdf} disabled={isPrinting} className="bg-green-600 hover:bg-green-700 text-white">
                        ...
                        <div className="p-6 bg-slate-200 overflow-auto max-h-[80vh] flex justify-center">
                            <div ref={receiptRef} className="bg-white p-5 relative overflow-hidden border border-slate-200 shadow-sm" style={{ width: "205.9mm", height: "130mm", boxSizing: "border-box" }}>
                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.04]"><FileCheck style={{ width: "200px", height: "200px", color: "#000" }} /></div>
                                <div className="relative z-10 h-full">
                                    <div className="text-center border-b-2 border-black pb-2 mb-3">
                                        <h1 className="text-2xl font-black uppercase tracking-tight text-black">{activeCompany?.name || "PARROQUIA PADRE MISERICORDIOSO"}</h1>
                                        <p className="text-[11px] font-semibold text-black mt-0.5">NIT: {activeCompany?.doc || "802012765"} | {activeCompany?.address || "CRA 9G # 77 - 42"} | Tel: {activeCompany?.phone || "3167630763"}</p>
                                    </div>
                                    <div className="flex justify-between items-end mb-3">
                                        <div>
                                            <h2 className="text-xl font-black tracking-widest text-black uppercase">RECIBO DE CAJA / DONACIÓN</h2>
                                            <p className="font-mono font-bold text-black text-sm mt-0.5">N° RC-{String(receiptToPrint?.voucherNumber).padStart(4, "0")}</p>
                                        </div>
                                        <div className="border border-slate-300 rounded-lg overflow-hidden flex flex-col text-center w-36 bg-slate-50">
                                            <div className="text-[10px] font-bold uppercase py-1 border-b border-slate-300 tracking-wider">FECHA</div>
                                            <div className="py-1.5 font-mono text-sm font-bold text-black">{receiptToPrint?.date ? format(parseISO(receiptToPrint.date), "dd 'de' MMMM, yyyy", { locale: es }) : ""}</div>
                                        </div>
                                    </div>
                                    <div className="space-y-3 pb-20">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-bold uppercase tracking-wider mb-1">RECIBIDO DE:</span>
                                            <div className="flex justify-between items-end border-b border-slate-300 pb-1">
                                                <span className="font-bold text-lg uppercase text-black truncate pr-2">{receiptToPrint?.beneficiary}</span>
                                                <span className="text-[11px] font-mono text-black shrink-0">NIT/CC: {receiptToPrint?.docNumber}</span>
                                            </div>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-bold text-green-700 uppercase tracking-wider mb-1">LA SUMA DE:</span>
                                            <div className="flex flex-col bg-green-50 border border-green-200 px-4 py-2 rounded-lg">
                                                <span className="font-mono text-3xl font-black text-black">${parseFloat(receiptToPrint?.amount || 0).toLocaleString("es-CO")}</span>
                                                <span className="text-[10px] font-bold uppercase mt-1 tracking-wide">{numeroALetras(parseFloat(receiptToPrint?.amount || 0))}</span>
                                            </div>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-bold uppercase tracking-wider mb-1">POR CONCEPTO DE:</span>
                                            <div className="border-b border-slate-300 pb-1 min-h-[1.5rem]"><p className="text-sm font-medium text-black leading-snug line-clamp-2">{receiptToPrint?.description}</p></div>
                                        </div>
                                    </div>
                                    <div className="absolute bottom-5 left-5 right-5 flex justify-between items-end">
                                        <div className="w-56 text-center">
                                            <div className="border-t border-black pt-1">
                                                <p className="font-bold text-[11px] uppercase text-black">FIRMA / SELLO RECIBIDO</p>
                                                <p className="text-[9px] text-black mt-0.5">{activeCompany?.name || "PARROQUIA PADRE MISERICORDIOSO"}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <DialogContent className="max-w-4xl p-0 border-none bg-transparent shadow-none">
                    <div className="bg-white rounded-lg overflow-hidden shadow-2xl">
                        <div className="p-4 border-b flex justify-between items-center bg-slate-50">
                            ...
                            <Button size="sm" onClick={handlePrintBillingDocPdf} disabled={isPrinting} className="bg-blue-600 hover:bg-blue-700 text-white">
                                {isPrinting ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Printer className="w-4 h-4 mr-2" />}
                                Imprimir PDF
                            </Button>
                        </div>
                        <div className="p-6 bg-slate-200 overflow-auto max-h-[80vh] flex justify-center">
                            <div ref={billingRef} className="bg-white p-5 relative overflow-hidden border border-slate-200 shadow-sm" style={{ width: "205.9mm", height: "130mm", boxSizing: "border-box" }}>
                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.04]"><FileText style={{ width: "200px", height: "200px", color: "#000" }} /></div>
                                <div className="relative z-10 h-full">
                                    <div className="text-center border-b-2 border-black pb-2 mb-3">
                                        <h1 className="text-2xl font-black uppercase tracking-tight text-black">{activeCompany?.name || "PARROQUIA PADRE MISERICORDIOSO"}</h1>
                                        <p className="text-[11px] font-semibold text-black mt-0.5">NIT: {activeCompany?.doc || "802012765"} | {activeCompany?.address || "CRA 9G # 77 - 42"} | Tel: {activeCompany?.phone || "3167630763"}</p>
                                    ...
                                    <div className="flex justify-between items-end mb-3">
                                        <div>
                                            <h2 className="text-xl font-black tracking-widest text-black uppercase">CUENTA DE COBRO</h2>
                                            <p className="font-mono font-bold text-black text-sm mt-0.5">Ref. Egreso N° E-{String(billingDocToPrint?.voucherNumber).padStart(4, "0")}</p>
                                        </div>
                                        <div className="border border-slate-300 rounded-lg overflow-hidden flex flex-col text-center w-36 bg-slate-50">
                                            <div className="text-[10px] font-bold uppercase py-1 border-b border-slate-300 tracking-wider text-black">FECHA</div>
                                            <div className="py-1.5 font-mono text-sm font-bold text-black">{billingDocToPrint?.date ? format(parseISO(billingDocToPrint.date), "dd 'de' MMMM, yyyy", { locale: es }) : ""}</div>
                                        </div>
                                    </div>
                                    <div className="space-y-3 pb-20">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-bold uppercase tracking-wider mb-1 text-black">DEBE A:</span>
                                            <div className="flex justify-between items-end border-b border-slate-300 pb-1">
                                                <span className="font-bold text-lg uppercase text-black truncate pr-2">{billingDocToPrint?.beneficiary}</span>
                                                <span className="text-[11px] font-mono text-black shrink-0">NIT/CC: {billingDocToPrint?.docNumber}</span>
                                            </div>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-bold text-blue-700 uppercase tracking-wider mb-1">LA SUMA DE:</span>
                                            <div className="flex flex-col bg-blue-50 border border-blue-200 px-4 py-2 rounded-lg">
                                                <span className="font-mono text-3xl font-black text-black">${parseFloat(billingDocToPrint?.amount || 0).toLocaleString("es-CO")}</span>
                                                <span className="text-[10px] font-bold uppercase mt-1 tracking-wide text-black">{numeroALetras(parseFloat(billingDocToPrint?.amount || 0))}</span>
                                            </div>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-bold uppercase tracking-wider mb-1 text-black">POR CONCEPTO DE:</span>
                                            <div className="border-b border-slate-300 pb-1 min-h-[1.5rem]"><p className="text-sm font-medium text-black leading-snug line-clamp-2 uppercase">{billingDocToPrint?.concept}</p></div>
                                        </div>
                                    </div>
                                    <div className="absolute bottom-5 left-5 right-5 flex justify-between items-end">
                                        <div className="w-56 text-center">
                                            <div className="border-t border-black pt-1">
                                                <p className="font-bold text-[11px] uppercase text-black truncate">{billingDocToPrint?.beneficiary}</p>
                                                <p className="text-[9px] text-black mt-0.5">Beneficiario / Contratista</p>
                                            </div>
                                        </div>
                                        <div className="w-56 text-center">
                                            <div className="border-t border-black pt-1">
                                                <p className="font-bold text-[11px] uppercase text-black">FIRMA AUTORIZADA</p>
                                                <p className="text-[9px] text-black mt-0.5">Aprobación / Contabilidad</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
