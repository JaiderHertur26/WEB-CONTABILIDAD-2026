...
const BookClosings = () => {
    ...
    const generateReport = () => {
        ...
        allRelevant.forEach(t => {
            ...
            // Registrar por separado los movimientos donde caja/banco actúa solo como puente.
            // No se llevan al resultado operativo: aquí solo se documentan sus entradas y salidas.
            if (isPuenteOCruce) {
                let puenteName = '';
                if (t.debitAccount && t.creditAccount) {
                    const drCode = String(t.debitAccount.code || '');
                    const crCode = String(t.creditAccount.code || '');
                    const drNameLocal = String(t.debitAccount.name || '').trim().toUpperCase();
                    const crNameLocal = String(t.creditAccount.name || '').trim().toUpperCase();
                    if (drCode.startsWith('2') || drNameLocal.includes('PUENTE') || drNameLocal.includes('CRUCE')) puenteName = drNameLocal;
                    else if (crCode.startsWith('2') || crNameLocal.includes('PUENTE') || crNameLocal.includes('CRUCE')) puenteName = crNameLocal;
                    else if (t.isInternalTransfer) puenteName = 'TRANSFERENCIA / CRUCE INTERNO';
                    else puenteName = crNameLocal || drNameLocal || 'CUENTA PUENTE';
                } else {
                    puenteName = String(t.category || '').trim().toUpperCase();
                    if (!puenteName) puenteName = t.isInternalTransfer ? 'TRANSFERENCIA / CRUCE INTERNO' : 'CUENTA PUENTE';
                }
                if (!puentesMap[puenteName]) puentesMap[puenteName] = { entrada: 0, salida: 0, movimientos: 0 };
                if (t.type === 'income') puentesMap[puenteName].entrada += amount;
                if (t.type === 'expense') puentesMap[puenteName].salida += amount;
                puentesMap[puenteName].movimientos += 1;
            }
        ...
        const puentesList = Object.entries(puentesMap)
            .map(([name, data]) => ({
                name,
                entrada: data.entrada,
                salida: data.salida,
                saldo: data.entrada - data.salida,
                movimientos: data.movimientos
            }))
            .filter(item => Math.abs(item.entrada) > 0.01 || Math.abs(item.salida) > 0.01)
            .sort((a, b) => (Math.abs(b.entrada) + Math.abs(b.salida)) - (Math.abs(a.entrada) + Math.abs(a.salida)));
        ...
        setReport({
            ...
            conciliacion: { tercerosIn, tercerosOut, capitalizacion, tercerosList, puentesList }
        });
    ...
    const executeExecutiveReportPrint = () => {
        ...
        const hasConciliacion = report.conciliacion?.tercerosIn > 0 || report.conciliacion?.tercerosOut > 0 || report.conciliacion?.capitalizacion > 0 || (report.conciliacion?.puentesList || []).length > 0;
        const saldoNetoTerceros = (report.conciliacion?.tercerosIn || 0) - (report.conciliacion?.tercerosOut || 0);
        const puentesList = report.conciliacion?.puentesList || [];
        ...
        const htmlContent = `
            ...
            <body>
                ...
                ${hasConciliacion ? `
                    <h3 class="section-heading">5. Cuentas Puente y Cruces</h3>
                    <p class="saludo" style="font-size: 10pt; color: #475569; margin-bottom: 8px;">
                        Movimientos en los que la caja o cuenta bancaria actúa únicamente como vehículo de paso. No forman parte de los ingresos ni de los gastos operativos.
                    </p>
                    ${puentesList.length > 0 ? `
                    <table style="width:100%;">
                        <thead><tr><th>Cuenta / Concepto</th><th style="text-align:right;">Entradas</th><th style="text-align:right;">Salidas</th><th style="text-align:right;">Saldo</th></tr></thead>
                        <tbody>
                        ${puentesList.map(item => `
                            <tr>
                                <td style="padding:5px 8px;">${item.name}</td>
                                <td style="text-align:right; font-family:monospace;">$${item.entrada.toLocaleString('es-ES', { minimumFractionDigits: 2 })}</td>
                                <td style="text-align:right; font-family:monospace;">$${item.salida.toLocaleString('es-ES', { minimumFractionDigits: 2 })}</td>
                                <td style="text-align:right; font-family:monospace; font-weight:bold;">$${item.saldo.toLocaleString('es-ES', { minimumFractionDigits: 2 })}</td>
                            </tr>`).join('')}
                        <tr style="background:#f1f5f9; font-weight:bold;">
                            <td style="padding:5px 8px;">TOTAL MOVIMIENTOS PUENTE</td>
                            <td style="text-align:right; font-family:monospace;">$${puentesList.reduce((s,i)=>s+i.entrada,0).toLocaleString('es-ES',{minimumFractionDigits:2})}</td>
                            <td style="text-align:right; font-family:monospace;">$${puentesList.reduce((s,i)=>s+i.salida,0).toLocaleString('es-ES',{minimumFractionDigits:2})}</td>
                            <td style="text-align:right; font-family:monospace;">$${puentesList.reduce((s,i)=>s+i.saldo,0).toLocaleString('es-ES',{minimumFractionDigits:2})}</td>
                        </tr>
                        </tbody>
                    </table>` : '<p style="font-size:10pt; color:#64748b;">No se registraron cuentas puente o cruces en el período.</p>'}

                    ${(report.conciliacion?.tercerosList || []).length > 0 ? `
                    <div style="margin-top:10px;">
                        <div style="font-weight:bold; margin-bottom:5px;">Fondos de terceros identificados</div>
                        <table style="width:100%;">
                            <thead><tr><th>Concepto</th><th style="text-align:right;">Monto</th></tr></thead>
                            <tbody>${report.conciliacion.tercerosList.map(item => `<tr><td style="padding:5px 8px;">${item.name}</td><td style="text-align:right; font-family:monospace; font-weight:bold;">$${item.total.toLocaleString('es-ES',{minimumFractionDigits:2})}</td></tr>`).join('')}</tbody>
                        </table>
                        <div style="margin-top:5px; text-align:right; font-weight:bold;">Saldo pendiente: $${saldoNetoTerceros.toLocaleString('es-ES',{minimumFractionDigits:2})}</div>
                    </div>` : ''}

                    ${report.conciliacion?.capitalizacion > 0 ? `
                    <div style="margin-top:10px; font-size:10pt;"><strong>Capitalización de activos:</strong> $${report.conciliacion.capitalizacion.toLocaleString('es-ES',{minimumFractionDigits:2})}</div>` : ''}
                ` : ''}
