...
const Transactions = () => {
    ...
    // La UI unifica los cruces para que el usuario no vea filas duplicadas, 
    // pero los mantenemos separados para la exportación.
        ...
        filteredTransactions.forEach(t => {
            ...
            if (t.isInternalTransfer) {
                ...
                if (sibling) {
                    ...
                    groups.push({ ...second, id: first.id, description: rawDesc, _mergedAmount: displayAmount, _isMerged: true, _sourceAccount: sourceAsset, _destAccount: destAsset, _rawAmount: amountVal, _destName: 'Cruce Contable' });
                } else { groups.push(t); }
            } else { groups.push(t); }
        });
        return groups;
    ...
    // 🚀 EXPORTACIÓN "CONTROL DE SALDOS": Iteramos sobre filteredTransactions (SIN FUSIONAR)
    // Esto garantiza que el Excel muestre exactamente a dónde fue y de dónde vino el dinero sin errores de strings.
    const handleExport = () => {
        if (filteredTransactions.length === 0) return;
        
        const dataToExport = filteredTransactions.map(t => {
            let typeLabel = (t.isInternalTransfer || t.type === 'transfer') ? 'Cruce/Transferencia' : (t.type === 'income' ? 'Ingreso' : 'Egreso');
            ...
            // Los egresos o salidas de transferencia son negativos
    ...
    // 🚀 EXPORTACIÓN "PARTIDA DOBLE": Lee los datos crudos (sin fusionar) igual que Reports.jsx
    const handleExportAccounting = () => {
        ...
        filteredTransactions.forEach(t => {
            ...
            // Si es Cruce o Transferencia, buscamos las dos caras de la moneda
            if (t.isInternalTransfer) {
                const baseId = t.id.replace(/-exp$|-inc$/, '');
                const isExp = t.id.endsWith('-exp');
                const siblingId = baseId + (isExp ? '-inc' : '-exp');
                ...
                if (sibling) {
                    ...
                    // Asiento perfecto: Débito a la cuenta que recibe (expensePart.category), Crédito a la cuenta que entrega (incomePart.category)
            ...
            // Asientos regulares (Ingresos/Egresos directos)
            let prefix = (t.type === 'transfer') ? 'T' : (t.type === 'income' ? 'I' : 'E');
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="bg-white">
                                    {displayTransactions.map(t => {
                                        // Ignore the dummy merged transaction, print the real journal entries directly from unmerged data
...
const AutoBillingConfigDialog = ({ open, onOpenChange, accounts, autoBillingCategories, onSave }) => {
    // ... (Mismo que tenías originalmente)
    return null;
};
